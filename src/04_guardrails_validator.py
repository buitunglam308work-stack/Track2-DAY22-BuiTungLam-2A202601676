...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value; nếu phát hiện, trả về FailResult kèm fix_value an toàn.
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            repaired_value = json.dumps(parsed, ensure_ascii=False, indent=2)
            return FailResult(
                error_message="JSON cần được chuẩn hóa",
                fix_value=repaired_value,
            )
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        ...
        # Print the complete validated output so repaired/fallback JSON is
        # auditable in the evidence log (input may remain truncated above).
        print(f"  Output: {result.validated_output}")


# ── 5. Main ────────────────────────────────────────────────────────────────
def main(argv=None):
    parser = argparse.ArgumentParser(description="Run Guardrails PII/JSON demos")
    parser.add_argument(
        "--demo",
        choices=("pii", "json", "all"),
        default="all",
        help="which validator demo to run (default: all)",
    )
